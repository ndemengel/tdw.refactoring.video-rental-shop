...
public class FakeMovieTable {
    ...
    static {
        ...
        newMovie("BEKINDREWI2008").title("Be Kind Rewind").country("US").releaseDate("2008/01/20").rentalReleaseDate("2008/04/26").genres("Comedy", "Drama")
                .duration(102).director("Michel Gondry").writers("Michel Gondry").cast("Jack Black", "Mos Def", "Danny Glover").ownedQuantity(5).insert();
        newMovie("THEDARKKNI2012").title("The Dark Knight Rises").country("US").releaseDate("2012/07/16").rentalReleaseDate(now.minusMonths(2))
        ...
        newMovie("AVENGERS2012").title("Avengers").country("US").releaseDate("2012/04/11").rentalReleaseDate(now.minusMonths(5)).genres("Action").duration(143)
                .director("Joss Whedon").writers("Zak Penn", "Joss Whedon").cast("Robert Downey Jr.", "Chris Evans", "Scarlett Johansson").ownedQuantity(17)
                .insert();
        newMovie("SOULKITCHE2009").title("Soul Kitchen").country("DE").releaseDate("2009/12/25").rentalReleaseDate("2010/08/01").genres("Comedy", "Drama")
        ...
        newMovie("LASOUPEAUX1981").title("La soupe aux choux").country("FR").releaseDate("1981/12/02").rentalReleaseDate("1985/11/05").genres("Comedy", "Sci-Fi")
    ...
    private static Function<Movie, Movie> copyMovie() {
        return new Function<Movie, Movie>() {
            public Movie apply(Movie in) {
                ...
                out.setRentalReleaseDate(in.getRentalReleaseDate());
    ...
    public static class MovieBuilder {
        ...
        public MovieBuilder rentalReleaseDate(String date) {
            movie.setRentalReleaseDate(parseDate(date));
            return this;
        }

        public MovieBuilder rentalReleaseDate(LocalDate date) {
            movie.setRentalReleaseDate(date.toDate());
            return this;
        }
